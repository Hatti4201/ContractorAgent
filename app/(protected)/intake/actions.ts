...
export async function analyzeInboxMessage(messageId: string) {
  ...
  // The source type is known rather than detected: this came out of the mailbox itself.
  const intake = await createIntakeFromMessage(messageId, await outlookAccessToken());
