  ...
  try {
    ...
    await getPrisma().outreachDraft.update({
      ...
      data: {
        ...
        outlookError: creationError?.message.slice(0, 500) ?? "Outlook draft creation failed; retry or reconnect.",
      },
    });
  }
  refresh(id);
...
export async function confirmOutlookSent(id: string) {
  ...
  try {
    result = await inspectOutlookSentMessage(draft.outlookMessageId, {
      toAddress: draft.toAddress,
      subject: draft.subject,
      resumePath: draft.attachmentResume.filePath,
  ...
  } else if (!result.matchesApprovedRouting) {
    await getPrisma().$transaction([
