...
test("resume routing uses one real enabled registry file and requires review below 70%", async () => {
  ...
  try {
    ...
    const resumes = [
      { id: "java", name: "Sample Java", roleFamily: "JAVA_BACKEND", filePath: javaPath, version: "v1", active: true },
      { id: "react", name: "Sample React", roleFamily: "REACT", filePath: reactPath, version: "v1", active: true },
      { id: "inactive", name: "Old Java", roleFamily: "JAVA_BACKEND", filePath: javaPath, version: "v0", active: false },
      { id: "missing", name: "Missing AI", roleFamily: "JAVA_AI", filePath: join(directory, "missing.pdf"), version: "v1", active: true },
    ];

    const confident = await buildResumeRoute("JAVA_BACKEND", 0.9, resumes);
    assert.equal(confident.recommended?.id, "java");
    assert.deepEqual(confident.candidates.map((resume) => resume.id), ["java", "react"]);

    const uncertain = await buildResumeRoute("JAVA_BACKEND", 0.69, resumes);
